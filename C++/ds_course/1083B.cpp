...
public:
    ...
    int Decode(const string codestr, char*& txtstr,map<string, char>mp);
    void Destroy();
};
// 构建huffman树
...
void HuffMan::Coding()
{
    ...
    for(i = 1; i <= lnum; ++ i)
    {
        ...
        for(int j = i, parent = huffTree[j].parent; parent != 0; j = parent, parent = huffTree[j].parent)
        {
            if(huffTree[parent].leftchild == j)
            {
                cd[--start] = '0';
            }
            else
...
int HuffMan::Decode(const string codestr, char*& txtstr, map<string, char>mp)
{
...
//主函数根据需要添加代码
    ...
    for(i = 0; i < t; i ++)
    {
        ...
        for(int j = 0; j < m; ++j)
        {
            ...
            cout << sizeof(ans);
            int res = myHuff.Decode(str, ans, mp);
            if(res == 1)
            {
                cout << ans << endl;
            }
            ...
            delete[]ans; // delete不掉是因为str.size()==5和AAAAA一样，越界了
        }
        myHuff.Destroy();
    }
    return 0;
